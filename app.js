...
function detail(w){
 ...
 app.querySelector('.detail .back-link').addEventListener('click',e=>{e.preventDefault();navigateBack();});app.querySelector('.detail-image').classList.toggle('natural-main',Boolean(w.mainNatural));const slots=app.querySelector('.media-slots');if(w.video)slots.innerHTML=`<section class="media-slot media-video"><h2 class="mono">${t('detailVideoTitle')}</h2><iframe src="${w.video}" title="${title} 影像记录" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></section>`;else slots.remove();if(w.hideTech)app.querySelectorAll('.detail-copy')[1]?.remove();
